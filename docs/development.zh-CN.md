...
- 菜单新增必须提交唯一 `code`、标题 `title` 和类型 `type`；`parent_id` 为空表示顶级菜单，且不得选择固定工作台作为父级。菜单图标为空时服务端统一回退为 `lucide:list`。`route_path` 是后台前端路由而不是 API 路径，服务端允许为空；自定义菜单前端可根据非空路由生成 `code`、`route_name` 和页面 `view_key`。父级不得指向自身或后代，系统菜单身份不可修改，固定工作台不可编辑或删除。
...
- 内置 `manager` 管理员角色只允许 `administrator` 超级管理员通过角色管理页编辑其菜单和权限授权；其他角色即使拥有 `system.role.update`，服务端也必须返回 `ADMIN_SUPER_ADMIN_REQUIRED`。管理员角色的标识、内置名称和启用状态固定。编辑器必须显示完整的菜单和权限树，包括菜单管理、权限管理及其操作权限；新安装的默认授权不勾选这些管理项，但超级管理员后续可以主动分配，系统同步必须保留非空的人工授权，不得再次强制撤销。
...
- 操作日志页面只读，服务端要求 `system.audit.view`。列表只返回摘要，详情接口返回完整的已脱敏变更和请求上下文；不提供删除接口。
