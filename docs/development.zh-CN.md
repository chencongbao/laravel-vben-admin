...
- 列表手动刷新统一使用 `src/components/system/list-refresh-button.vue`。它与顶栏刷新按钮共同调用Vben `useRefresh()`，只重新渲染当前路由页面，不刷新整个浏览器；两处入口必须保持完全相同的局部页面刷新语义。自动刷新继续直接调用列表加载函数并保持静默。页面不得自行实现第三套手动刷新逻辑。
...
- 后台公共视觉规则统一维护在 `src/styles/admin-ui.css`（按钮、列表工具栏、搜索Card、搜索字段、行操作和批量操作）与 `src/styles/admin-table.css`（Table）中；公共组件和业务页面只输出结构及约定类名，不得在单个Vue文件内重复定义或覆盖这些样式。所有Ant Design按钮统一使用直角样式，圆形状态指示、头像、开关等非按钮元素不受此规则影响。
