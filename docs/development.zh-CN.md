...
- 用户删除使用独立的 `system.user.delete` 权限并记录 `system.user.deleted` 审计。管理员和超级管理员可以删除普通角色用户；任何持有 `manager` 或 `administrator` 角色的账号都属于受保护账号，前端不显示删除按钮，服务端直接调用返回 `PROTECTED_ADMIN_USER_DELETE_DENIED`。删除普通用户时同步撤销其访问令牌，角色关联由外键级联清理。
