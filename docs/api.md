...
The role management UI creates and updates a non-super role with `code`, `name`, `permission_ids`, and `menu_ids` in one atomic request. Role status and permission/menu counts are not exposed in the role list or editor. The existing `is_active` database field remains enforced by the server, and newly created roles use its enabled default. The built-in `administrator` role is immutable and has implicit full access. The built-in `manager` identity is protected, while its access assignments can be updated. A role assigned to administrators cannot be deleted.
...
On a fresh installation, `manager` receives every built-in system permission and every built-in system menu except the fixed workspace. The workspace is available to every authenticated administrator without a role-menu pivot. The `administrator` role remains implicit-full-access and does not require duplicated role-permission or role-menu rows. Synchronization fills the manager defaults only when the corresponding existing assignment set is empty, preserving non-empty customized assignments.
