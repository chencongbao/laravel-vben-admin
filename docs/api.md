...
Every failed administrator login is persisted to the database login log before an asynchronous Telegram alert is requested. Login failures do not write an additional local Foundation exception log. Alerts contain only the username, matched administrator ID when available, stable failure code, IP, method, path, truncated User-Agent and optional request ID; passwords, authorization headers, cookies and Telegram credentials are never included. Set `VBEN_ADMIN_ALERT_LOGIN_FAILURES=false` to disable this alert without disabling database login logging.
...
The administration SPA persists its Bearer token and access-code cache only in the current tab's `sessionStorage`. Closing the tab clears that browser persistence. The local screen-lock password is memory-only and is never persisted to Web Storage; refreshing the page therefore clears the local screen lock rather than storing a reusable password in the browser.
...
`DELETE /system/users/{adminUser}` requires `system.user.delete`. Only the two fixed built-in identities, `cmsadmin` and `admin`, are deletion-protected. Every account created later remains deletable even when it is assigned the built-in `manager` role. Non-super administrators still cannot delete an account that holds a super-administrator role; that data-scope violation returns `ADMIN_PRIVILEGE_ESCALATION_DENIED`. Successful deletion revokes the target's access tokens, relies on the user-role foreign key to remove role bindings, and records `system.user.deleted`.

The fixed 12-character sentence above describes the default `strong` policy. When `system.password_strength` is set to `weak`, administrator creation and password reset instead require at least 6 characters without a character-combination requirement. The form help text follows the active policy.
...
On a fresh installation, `manager` receives the built-in security-center menu, the `system.security.view` and `system.security.update` permissions, and `system.setting.view`, which allows both viewing and changing system settings. Its initial assignment omits the fixed workspace, menu management, permission management, and theme configuration: `system.menus`, `system.permissions`, `system.theme-settings`, `system.menu.*`, `system.permission.*`, and `system.theme-setting.view`. The configuration parent remains available through system settings. The workspace is available to every authenticated administrator without a role-menu pivot. The `administrator` role remains implicit-full-access and does not require duplicated role-permission or role-menu rows. Synchronization removes the obsolete `system.setting.update` and `system.theme-setting.update` permissions. It fills default manager assignments only when the corresponding assignment set is empty and preserves every other non-empty customized assignment.

`GET /system/roles` and `GET /system/permissions` accept an optional positive integer `id` query parameter for exact identifier filtering. Super administrators can list and view every role. All other administrators cannot list or directly view the built-in `administrator` and `manager` roles. Administrator role selectors use the dedicated `/system/users/role-options` scope: super administrators receive every active non-super role, while other administrators receive only active roles whose permission and menu assignments are subsets of their own effective assignments. User create and update enforce the same boundary on the server, and no user-management request may assign a super-administrator role.
...
Menu create requires unique `code`, `title`, and `type`; `parent_id` is nullable for a root menu, the fixed workspace cannot be used as a parent, and a blank icon falls back to `lucide:list`. For directory and page menus, `route_path` is a nullable frontend route rather than an API path. An `external` menu requires a complete `http://` or `https://` URL in `route_path`; unsafe schemes are rejected, `view_key` is cleared, and the effective-menu response exposes the URL through `meta.link` with `meta.openInNewWindow=true` while using a stable internal placeholder route. Menu create/update accepts optional `permission_ids` with at most 500 distinct existing IDs.
...
`GET /system/audit-logs/actions` returns every registered stable operation code as `{code, type, module, label_key}` objects. `label_key` is an i18n key rather than translated text, so every installed frontend language can translate the same definition. The administration client displays all registered actions as localized, searchable business-action options and submits only `code` through the existing `action` filter. The log list also accepts `action_type` with `created`, `updated`, `deleted`, or `other`, exposed as a localized searchable select. Unknown historical codes remain queryable by API and fall back to their original code without blocking logging. The operation-log list omits the change-count column; complete redacted changes remain available from the detail endpoint.

Operator options are role-scoped: super administrators receive all current administrator users, while non-super administrators receive all current users except super administrators. Business-action options are the complete registered catalog for every account authorized with `system.audit.view`. Log data remains independently scoped: super administrators can view all login and operation logs; every non-super administrator is excluded from records caused by a super administrator and cannot retrieve a hidden operation-log detail directly by ID.
...
Audit changes and context recursively redact keys containing password, token, secret, credential, authorization, cookie, private key, captcha, TOTP, or two-factor data. Captured request input is deny-by-default: each registered action may define a `request_fields` allowlist in `config/vben-admin-log.php`, and unregistered actions capture no request fields. Allowed values still pass through recursive redaction. Uploaded files are represented only by safe metadata. Login and operation records are read-only through these APIs.
