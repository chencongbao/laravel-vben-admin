...
On a fresh installation, `manager` receives the built-in security-center menu, the `system.security.view` and `system.security.update` permissions, and both `system.setting.view` and `system.setting.update`. Its initial assignment omits the fixed workspace, menu management, permission management, and theme configuration: `system.menus`, `system.permissions`, `system.theme-settings`, `system.menu.*`, `system.permission.*`, `system.theme-setting.view`, and `system.theme-setting.update`. The configuration parent remains available through system settings. The workspace is available to every authenticated administrator without a role-menu pivot. The `administrator` role remains implicit-full-access and does not require duplicated role-permission or role-menu rows. Synchronization fills these default manager assignments only when the corresponding assignment set is empty and preserves every non-empty customized assignment, including assignments explicitly changed later by a super administrator.
